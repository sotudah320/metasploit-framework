  ...
  def load_resource(path)
	...
	def parse_multi_line(line, parse_opts = {})
		...
		parse_defaults = {
							...
							:quote_char       => '"',
							:skip_blanks      => true,
							:field_size_limit => 1024,
              # this effectively sets quoted command size limit after
              # field_size_limit, if a matching quote_char isn't found then a
              # CSV::MalformedCSVError is raised
						}
    ...
    # CSV.parse will return an Array of Rows (which are Arrays when no headers
    # are used), and since we removed new lines, we always have an Array of 1
    # Array, so we send :first method above
	end
